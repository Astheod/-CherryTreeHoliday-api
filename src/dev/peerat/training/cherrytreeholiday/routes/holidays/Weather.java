...
public class Weather implements Response {
	...
	public Weather(Repository repo) {
		this.repo = repo;
	...
	@Route(path = "/holiday/(\\d+)/weather/", needLogin = true)
	public void exec(Matcher matcher, Context context, HttpReader reader, HttpWriter writer) throws Exception{
		...
		if(param != null){
			...
			Address address = holiday.getAddress();

			String country = address.getCountry();
			HttpURLConnection call = (HttpURLConnection) new URL("https://api.openweathermap.org/data/2.5/weather?q=" + address.getCity() + (map.containsKey(country) ? "," + map.get(country) : "") + "&units=metric&lang=fr&appid=5128edbe2e5f1aa9c7fb0a53cdb605c4").openConnection();
			BufferedReader breader = new BufferedReader(new InputStreamReader(call.getInputStream()));
			String value = "";
			String line;
			while((line = breader.readLine()) != null) value+=line;
			breader.close();
			JsonMap json = new JsonParser().parse(value);
			
			context.response(200);
			JsonMap map = new JsonMap();
			map.set("main", json.get("main"));
			map.set("weather", json.get("weather"));
